...
void notify_all_clients(int sock, const char *message);
...
void notify_all_clients(int sock, const char *message)
{
	...
	for (int i = 0; i < num_clients; i++)
	{
		if (clients[i].active)
		{
			...
			{
				if (sendto(sock, full_msg, strlen(full_msg), 0, (struct sockaddr *)&clients[i].addr, sizeof(clients[i].addr)) >= 0)
					break;
				perror("Ошибка отправки уведомления клиенту, повторная попытка");
				sleep(1);
			}
		}
	}
...
int main(int argc, char *argv[])
{
	...
	// Уведомление всех клиентов о завершении работы сервера
	notify_all_clients(sock, "SERVER_SHUTDOWN");
	printf("Отправлено уведомление о завершении работы всем клиентам\n");
	sleep(2); // Даем время клиентам на обработку
