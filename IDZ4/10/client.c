...
int send_receive(int sock, struct sockaddr_in *server_addr, char *send_msg, char *recv_buffer)
{
	...
	{
		printf("Отправка: %s", send_msg); // Отладка
		sendto(sock, send_msg, strlen(send_msg), 0, (struct sockaddr *)server_addr, addr_len);

		struct pollfd fd = {.fd = sock, .events = POLLIN};
		int ready = poll(&fd, 1, TIMEOUT_SEC * 1000);
		if (ready > 0)
		{
			int n = recvfrom(sock, recv_buffer, BUFFER_SIZE - 1, 0, (struct sockaddr *)server_addr, &addr_len);
			if (n > 0)
			{
				recv_buffer[n] = '\0';
				printf("Получено: %s", recv_buffer); // Отладка
				if (strcmp(recv_buffer, "SERVER_SHUTDOWN\n") == 0)
				{
					printf("Получено уведомление о завершении сервера, завершение работы...\n");
					close(sock);
					exit(0);
				}
				return n;
...
int main(int argc, char *argv[])
{
	...
	// Ожидание уведомления о завершении сервера
	while (1)
	{
		struct pollfd fd = {.fd = sock, .events = POLLIN};
		if (poll(&fd, 1, TIMEOUT_SEC * 1000) > 0)
		{
