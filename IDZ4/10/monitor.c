...
int main(int argc, char *argv[])
{
	...
	// Регистрация как монитора (или повторное подключение)
	...
	while (1)
	{
		...
		if (fds[0].revents & POLLIN)
		{
			...
			int n = recvfrom(sock, buffer, BUFFER_SIZE - 1, 0, (struct sockaddr *)&from_addr, &from_len);
			if (n <= 0)
				break;
			buffer[n] = '\0';
			if (strcmp(buffer, "SERVER_SHUTDOWN\n") == 0)
			{
				print_with_timestamp("Получено уведомление о завершении сервера, завершение работы...\n");
				break;
			}
