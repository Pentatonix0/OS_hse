...
// Функция для логирования сообщений на сервере
void log_message(const char *message)
{
	time_t now = time(NULL);
	char *timestamp = ctime(&now);
	timestamp[strlen(timestamp) - 1] = '\0'; // Удаляем перенос строки
	printf("[%s] %s", timestamp, message);
}

// Функция для отправки таблицы сада мониторам
	...
	for (int i = 0; i < monitor_count; i++)
	{
		if (monitor_socks[i] != -1)
		{
			if (send(monitor_socks[i], timed_message, strlen(timed_message), 0) < 0)
			{
				...
				monitor_socks[i] = -1; // Помечаем для удаления
			}
		}
	}
	// Очистка невалидных сокетов
	int j = 0;
	for (int i = 0; i < monitor_count; i++)
	{
		if (monitor_socks[i] != -1)
...
void print_grid_to_all_monitors()
{
	...
	log_message(grid_str); // Логируем таблицу на сервере
}
...
void *handle_client(void *arg)
{
	...
	// Получение идентификации клиента
	...
	buffer[n] = '\0';
	// printf("[DEBUG] Получено: %s", buffer);
	...
	if (sscanf(buffer, "ID %d", &gardener_id) == 1 && gardener_id >= 1 && gardener_id <= 2)
	{
		...
		// printf("[DEBUG] Отправлено садовнику %d: OK\n", gardener_id);
	...
	else if (strcmp(buffer, "MONITOR\n") == 0)
	{
		...
		// printf("[DEBUG] Отправлено монитору: OK\n");
	...
	while (1)
	{
		...
		buffer[n] = '\0';
		// printf("[DEBUG] Получено от %s %d: %s", is_monitor ? "монитора" : "садовника", is_monitor ? 0 : gardener_id, buffer);
		...
		if (is_monitor)
		{
			if (strcmp(buffer, "GET_GRID\n") == 0)
			{
				...
				log_message(msg);
				// printf("[DEBUG] Отправка сетки монитору\n");
			...
			else
			{
				...
				log_message(msg);
				// printf("[DEBUG] Игнорируется неизвестная команда монитора\n");
			}
		}
		else
		{
...
int main(int argc, char *argv[])
{
	...
	// Инициализация сокетов мониторов
	...
	while (1)
	{
		...
		*client_sock = accept(server_sock, NULL, NULL);
		...
		pthread_detach(thread); // Отсоединяем поток для избежания утечек памяти
	}
