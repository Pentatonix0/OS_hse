...
int main(int argc, char *argv[])
{
	...
	if (n <= 0)
	{
		perror("Receive ID response failed");
		close(sock);
		return 1;
	}
	buffer[n] = '\0';
	...
	for (int k = 0; k < M * N; k++)
	{
		...
		while (attempts < max_attempts)
		{
			...
			{
				printf("Сервер завершает работу, садовник %d завершает задачу\n", id);
				close(sock);
				return 0;
			}
		...
		{
			printf("Садовник %d не смог переместиться в (%d, %d), пропуск\n", id, x, y);
			continue;
		}
		sprintf(buffer, "GET_STATE %d %d\n", x, y);
		if (send(sock, buffer, strlen(buffer), 0) < 0)
		{
			perror("Send GET_STATE failed");
			close(sock);
			return 1;
		}
		n = recv(sock, buffer, sizeof(buffer) - 1, 0);
		...
		if (T == P)
		{
			...
			if (send(sock, buffer, strlen(buffer), 0) < 0)
			{
				perror("Send FINISH_PROCESSING failed");
				close(sock);
				return 1;
			}
			n = recv(sock, buffer, sizeof(buffer) - 1, 0);
